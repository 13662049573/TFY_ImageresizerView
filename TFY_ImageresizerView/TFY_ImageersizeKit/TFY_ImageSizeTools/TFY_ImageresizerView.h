 ...
 * 可配置初始化（configure）：
    ...
    isShowGridlinesWhenIdle --- 闲置时是否能继续显示网格线（frameType 为 TFY_ClassicFrameType 且 gridCount > 1 且 maskImage 为 nil 才显示网格）
...
/*!
 ...
 @discussion 可使用TFY_ImageresizerConfigure配置好初始参数创建实例
 */
...
/*!
 @method
 @brief 取消视频导出
 @discussion 当视频正在导出时（裁剪、修正方向）调用即可取消导出，触发errorBlock回调（TFY_IEReason_ExportCancelled）
 */
