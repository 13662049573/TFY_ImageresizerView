...
typedef NS_ENUM(NSUInteger, TFY_DotRegion) {
    TFY_Center,
    TFY_LeftTop,
    TFY_LeftMid,
    TFY_LeftBottom,
...
@interface TFY_ImageresizerFrame ()
@property (nonatomic, strong) NSTimer *timer;

@property (nonatomic, weak) UIScrollView *scrollView;
@property (nonatomic, weak) UIImageView *imageView;
@property (nonatomic, weak) TFY_ImageresizerBlur *blurView;
@property (nonatomic, weak) TFY_ImageresizerBlur *maskBlurView;
...
@implementation TFY_ImageresizerFrame
{
    ...
    if (isAnimated) {
        ...
        if (_borderImage) {
            ...
            void (^layerOpacityAnimate)(CALayer *layer, id toValue) = ^(CALayer *layer, id toValue) {
                if (!layer) return;
                [layer tfy_addBackwardsAnimationWithKeyPath:@"opacity" fromValue:@(layer.opacity) toValue:toValue timingFunctionName:timingFunctionName duration:duration];
            };
            layerOpacityAnimate(_frameLayer, @(opacity));
            layerOpacityAnimate(_dotsLayer, @(otherOpacity));
            if (_frameType == TFY_ClassicFrameType && _maskImage == nil) layerOpacityAnimate(_gridlinesLayer, @(otherOpacity));
        }
    } else {
        [self.blurView setIsMaskAlpha:!isPreview duration:0];
        [self.maskBlurView setIsMaskAlpha:!isPreview duration:0];
...
- (void)setFrameType:(TFY_ImageresizerFrameType)frameType {
    ...
    if (frameType == TFY_ConciseFrameType) {
        [_gridlinesLayer removeFromSuperlayer];
...
- (void)setGridCount:(NSUInteger)gridCount {
    ...
    if (_borderImage || _isPreview || _frameType != TFY_ClassicFrameType) return;
    if (!CGRectIsEmpty(_imageresizerFrame)) {
        [self __updateImageresizerFrame:_imageresizerFrame animateDuration:0];
    }
}
...
- (BOOL)__isHorizontalDirection:(TFY_ImageresizerRotationDirection)direction {
    return (direction == TFY_ImageresizerHorizontalLeftDirection ||
            direction == TFY_ImageresizerHorizontalRightDirection);
}
...
- (void)__updateShapeLayersStrokeColor {
    ...
    if (_frameType == TFY_ConciseFrameType) {
...
- (void)__updateRotationDirection:(TFY_ImageresizerRotationDirection)rotationDirection {
...
- (void)__updateMaxResizeFrameWithDirection:(TFY_ImageresizerRotationDirection)direction {
    ...
    if (_frameType == TFY_ClassicFrameType) {
...
- (void)__checkIsCanRecovery {
    ...
    BOOL isOriginFrame = self.rotationDirection == TFY_ImageresizerVerticalUpDirection && [self __imageresizerFrameIsEqualImageViewFrame];
