...
@implementation TFY_ImageresizerTool{
    BOOL _isFixedOrientation;
}
#pragma mark - 计算函数

static BOOL TFY_IsSameSize(CGSize size1, CGSize size2) {
    return (fabs(size1.width - size2.width) < 0.5 && fabs(size1.height - size2.height) < 0.5);
}

static CGRect TFY_ConfirmCropFrame(CGRect cropFrame, CGSize resizeContentSize, CGFloat resizeWHScale, CGSize originSize) {
    if (TFY_IsSameSize(cropFrame.size, resizeContentSize)) {
...
static CGAffineTransform TFY_ConfirmTransform(CGSize originSize, TFY_ImageresizerRotationDirection direction, BOOL isVerMirror, BOOL isHorMirror, BOOL isImage) {
...
static CGPoint TFY_ConfirmTranslate(CGRect cropFrame, CGSize originSize, TFY_ImageresizerRotationDirection direction, BOOL isVerMirror, BOOL isHorMirror, BOOL isImage) {
    if (TFY_IsSameSize(cropFrame.size, originSize)) {
        return CGPointZero;
    }
    CGFloat tx = cropFrame.origin.x;
    CGFloat ty = cropFrame.origin.y;
    switch (direction) {
        case TFY_ImageresizerVerticalUpDirection:
            // 原点在左上
            if (isVerMirror || isHorMirror) {
                if (isVerMirror && isHorMirror) {
                    return TFY_ConfirmTranslate(cropFrame, originSize, TFY_ImageresizerVerticalDownDirection, NO, NO, isImage);
                } else if (isVerMirror) {
                    if (isImage) {
        ...
        case TFY_ImageresizerHorizontalLeftDirection:
            ...
            if (isVerMirror || isHorMirror) {
                if (isVerMirror && isHorMirror) {
                    return TFY_ConfirmTranslate(cropFrame, originSize, TFY_ImageresizerHorizontalRightDirection, NO, NO , isImage);
                } else if (isVerMirror) {
                    if (isImage) {
                        tx = originSize.width - CGRectGetMaxX(cropFrame);
                    } else {
        ...
        case TFY_ImageresizerVerticalDownDirection:
            ...
            if (isVerMirror || isHorMirror) {
                if (isVerMirror && isHorMirror) {
                    return TFY_ConfirmTranslate(cropFrame, originSize, TFY_ImageresizerVerticalUpDirection, NO, NO, isImage);
                } else if (isVerMirror) {
                    if (isImage) {
                        tx = -tx;
                    } else {
        ...
        case TFY_ImageresizerHorizontalRightDirection:
            ...
            if (isVerMirror || isHorMirror) {
                if (isVerMirror && isHorMirror) {
                    return TFY_ConfirmTranslate(cropFrame, originSize, TFY_ImageresizerHorizontalLeftDirection, NO, NO, isImage);
                } else if (isVerMirror) {
                    if (isImage) {
                        tx = -tx;
                        ty = -(originSize.height - CGRectGetMaxY(cropFrame));
...
static NSTimeInterval TFY_ImageSourceGetGIFFrameDelayAtIndex(CGImageSourceRef source, size_t index) {
...
static CGImageRef TFY_CreateNewCGImage(CGImageRef imageRef, CGContextRef context, UIImage *maskImage, BOOL isRoundClip, CGRect renderRect, CGAffineTransform transform, CGSize imageSize) {
    if (maskImage) {
...
+ (void)__executeErrorBlock:(TFY_ImageresizerErrorBlock)block cacheURL:(NSURL *)cacheURL reason:(TFY_ImageresizerErrorReason)reason {
    if (!block) return;
    dispatch_async(dispatch_get_main_queue(), ^{
        block(cacheURL, reason);
    });
}
+ (void)__executeCropPictureDoneBlock:(TFY_CropPictureDoneBlock)block finalImage:(UIImage *)finalImage cacheURL:(NSURL *)cacheURL isCacheSuccess:(BOOL)isCacheSuccess {
    if (!block) return;
    dispatch_async(dispatch_get_main_queue(), ^{
        block(finalImage, isCacheSuccess ? cacheURL : nil, isCacheSuccess);
    });
}
+ (void)__executeExportVideoStart:(TFY_ExportVideoStartBlock)block exportSession:(AVAssetExportSession *)exportSession {
    if (!block) return;
    dispatch_async(dispatch_get_main_queue(), ^{
        block(exportSession);
    });
}
+ (void)__executeExportVideoCompleteBlock:(TFY_ExportVideoCompleteBlock)block cacheURL:(NSURL *)cacheURL {
    if (!block) return;
    dispatch_async(dispatch_get_main_queue(), ^{
        block(cacheURL);
    });
...
+ (void)__cropPicture:(UIImage *)image
    ...
    if ((!image && !imageData) || compressScale <= 0) {
        [self __executeErrorBlock:errorBlock cacheURL:nil reason:TFY_IEReason_NilObject];
        return;
    }
    ...
    TFY_ImageresizerRotationDirection direction = configure.direction;
    ...
    if (cacheURL) {
        ...
        if ([[NSFileManager defaultManager] fileExistsAtPath:cacheURL.path]) {
            [self __executeErrorBlock:errorBlock cacheURL:cacheURL reason:TFY_IEReason_CacheURLAlreadyExists];
            return;
        }
    }
    
    if (TFY_IsSameSize(cropFrame.size, resizeContentSize) &&
        direction == TFY_ImageresizerVerticalUpDirection &&
        isVerMirror == NO &&
        isHorMirror == NO &&
        maskImage == nil &&
        isRoundClip == NO &&
    ...
    cropFrame = TFY_ConfirmCropFrame(cropFrame, resizeContentSize, resizeWHScale, imageSize);
    
    CGRect renderRect = (CGRect){CGPointZero, cropFrame.size};
    if (direction == TFY_ImageresizerHorizontalLeftDirection || direction == TFY_ImageresizerHorizontalRightDirection) {
        CGFloat tmp = renderRect.size.width;
        renderRect.size.width = renderRect.size.height;
        renderRect.size.height = tmp;
    }
    
    CGAffineTransform transform = TFY_ConfirmTransform(imageSize, direction, isVerMirror, isHorMirror, YES);
    CGPoint translate = TFY_ConfirmTranslate(cropFrame, imageSize, direction, isVerMirror, isHorMirror, YES);
    ...
    if (isGIF && index < 0) {
        ...
        if (imageData) {
            getCurrentImageRef = ^(size_t i) {
                ...
                NSTimeInterval delay = TFY_ImageSourceGetGIFFrameDelayAtIndex(source, i) / rate;
                duration += delay;
        ...
        void (^createNewImageRef)(size_t i) = ^(size_t i) {
            @autoreleasepool {
                ...
                // 绘制裁剪后的图片
                CGImageRef newImageRef = TFY_CreateNewCGImage(getCurrentImageRef(i),
                                                            context,
                                                            maskImage,
                                                            isRoundClip,
                                                            renderRect,
    ...
    CGImageRef newImageRef = TFY_CreateNewCGImage(imageRef, context, maskImage, isRoundClip, renderRect, transform, imageSize);
...
+ (UIImage *)decodeGIFData:(NSData *)data {
    ...
    for (size_t i = 0; i < count; i++) {
        NSTimeInterval delay = TFY_ImageSourceGetGIFFrameDelayAtIndex(source, i);
        totalTime += delay;
...
+ (void)cropVideoWithAsset:(AVURLAsset *)asset
    ...
    if (!asset) {
        [self __executeErrorBlock:errorBlock cacheURL:cacheURL reason:TFY_IEReason_NilObject];
        return;
    }
    ...
    if (!fileType) {
        [self __executeErrorBlock:errorBlock cacheURL:cacheURL reason:TFY_IEReason_NoSupportedFileType];
        return;
    }
    ...
    AVAssetTrack *videoTrack = [asset tracksWithMediaType:AVMediaTypeVideo].firstObject;
    if (!videoTrack) {
        [self __executeErrorBlock:errorBlock cacheURL:cacheURL reason:TFY_IEReason_VideoAlreadyDamage];
        return;
    }
    ...
    cropFrame = TFY_ConfirmCropFrame(cropFrame, resizeContentSize, resizeWHScale, videoSize);
    ...
    // 确定形变矩阵
    CGAffineTransform transform = TFY_ConfirmTransform(videoSize, direction, isVerMirror, isHorMirror, NO);
    CGPoint translate = TFY_ConfirmTranslate(cropFrame, videoSize, direction, isVerMirror, isHorMirror, NO);
    transform = CGAffineTransformTranslate(transform, translate.x, translate.y);
    ...
    [session exportAsynchronouslyWithCompletionHandler:^{
        ...
        if (status == AVAssetExportSessionStatusCompleted) {
            ...
            [self __executeErrorBlock:errorBlock cacheURL:cacheURL reason:(status == AVAssetExportSessionStatusCancelled ?  TFY_IEReason_VideoExportCancelled : TFY_IEReason_VideoExportFailed)];
        }
    }];
}
...
+ (void)fixOrientationVideoWithAsset:(AVURLAsset *)asset
    ...
    if (!asset) {
        [self __executeErrorBlock:fixErrorBlock cacheURL:nil reason:TFY_IEReason_NilObject];
        return;
    }
    ...
    if (!fileType) {
        [self __executeErrorBlock:fixErrorBlock cacheURL:nil reason:TFY_IEReason_NoSupportedFileType];
        return;
    }
    ...
    AVAssetTrack *videoTrack = [asset tracksWithMediaType:AVMediaTypeVideo].firstObject;
    if (!videoTrack) {
        [self __executeErrorBlock:fixErrorBlock cacheURL:nil reason:TFY_IEReason_VideoAlreadyDamage];
        return;
    }
    ...
    [session exportAsynchronouslyWithCompletionHandler:^{
        ...
        if (status == AVAssetExportSessionStatusCompleted) {
            ...
            [[NSFileManager defaultManager] removeItemAtURL:tmpURL error:nil];
            [self __executeErrorBlock:fixErrorBlock cacheURL:nil reason:(status == AVAssetExportSessionStatusCancelled ?  TFY_IEReason_VideoExportCancelled : TFY_IEReason_VideoExportFailed)];
        }
    }];
}
...
- (void)setVideoObj:(TFY_ImageresizerTool *)videoObj {
    _videoObj = videoObj;
