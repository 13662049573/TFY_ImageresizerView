...
@interface TFY_PhotoTool ()<PHPhotoLibraryChangeObserver>
...
+ (void)initialize {
    ...
    dispatch_once(&onceToken, ^{
        thumbnailPhotoSize_ = CGSizeMake(300, 300);
    });
}

#pragma mark - singleton

TFY_SingtonImplement(TFY_PhotoTool)
            ...
            switch (status) {
                case PHAuthorizationStatusDenied:       // 还不确定
                {
                    if (oldStatus != PHAuthorizationStatusNotDetermined) {
                        // 已经点了拒绝
                        [self __jurisdictionAlertWithTitle:@"没有访问照片的权限" message:@"请前往设置开启照片访问权限" isAlbum:YES fromVC:fromVC cancelHandle:^{
                        ...
                        if (refuseBlock) {
                            ...
                            [self __jurisdictionAlertWithTitle:@"您已拒绝访问照片的权限" message:@"需要开启照片访问权限才可以打开相册" isAlbum:YES fromVC:fromVC cancelHandle:^{
                                !alreadyRefuseBlock ? : alreadyRefuseBlock();
                            }];
                        }
                    }
...
- (void)cameraAuthorityWithAllowAccessAuthorityHandler:(void (^)(void))allowBlock
    ...
    if (![UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera]) {
        if (canNotBlock) {
            ...
            // 若不可用则退出方法
            [self __jurisdictionAlertWithTitle:@"提示" message:@"您的相机不能使用" isAlbum:YES fromVC:fromVC cancelHandle:^{}];
        }
        return;
    }
    ...
    if (authStatus == AVAuthorizationStatusNotDetermined) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if (granted) {
                    ...
                    if (refuseBlock) {
                        ...
                        [self __jurisdictionAlertWithTitle:@"您已拒绝访问相机的权限" message:@"需要开启相机访问权限才可以使用相机" isAlbum:NO fromVC:fromVC cancelHandle:^{
                            !alreadyRefuseBlock ? : alreadyRefuseBlock();
                        }];
                    }
                }
        ...
        if (authStatus == AVAuthorizationStatusAuthorized) {
            ...
            [self __jurisdictionAlertWithTitle:@"没有访问相机的权限" message:@"需要开启相机访问权限才可以使用相机" isAlbum:NO fromVC:fromVC cancelHandle:^{
                !alreadyRefuseBlock ? : alreadyRefuseBlock();
            }];
        }
    }
    ...
    UIAlertAction *action = [UIAlertAction actionWithTitle:@"前往开启" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        if ([[UIApplication sharedApplication] canOpenURL:url]) {
            if (@available(iOS 10.0, *)) {
             ...
             [self __jurisdictionAlertWithTitle:@"提示" message:@"无法前往设置页面" isAlbum:YES fromVC:fromVC cancelHandle:^{}];
        }
    }];
...
- (void)getAllAssetCollectionWithFastEnumeration:(TFY_AssetCollectionFastEnumeration)fastEnumeration
    ...
    !fastEnumeration ? : fastEnumeration([PHAssetCollection new], 0, allPhotoAlbumsFR.count);
...
- (void)requestLivePhotoForAsset:(PHAsset *)asset
    ...
    [[PHImageManager defaultManager] requestLivePhotoForAsset:asset targetSize:targetSize contentMode:PHImageContentModeAspectFill options:(options ? options : 0) resultHandler:^(PHLivePhoto * _Nullable livePhoto, NSDictionary * _Nullable info) {
        if (info[PHImageErrorKey] || [info[PHImageCancelledKey] boolValue]) {
            resultHandler(asset, livePhoto, NO);
            return;
        }
